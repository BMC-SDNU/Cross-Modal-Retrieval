...
class EncoderImagePrecomp(nn.Module):
    ...
    def __init__(self, img_dim, embed_size, no_imgnorm=False):
        ...
        self.ul = nn.Linear(img_dim, embed_size)
        self.ug = nn.Linear(embed_size, embed_size)
    ...
    def init_weights(self):
        """Xavier initialization for the fully connected layer
        """
        r = np.sqrt(6.) / np.sqrt(self.ul.in_features +
                                  self.ul.out_features)
        self.wl.weight.data.uniform_(-r, r)
        self.wl.bias.data.fill_(0)

        self.ul.weight.data.uniform_(-r, r)
        self.ul.bias.data.fill_(0)
        self.ug.weight.data.uniform_(-r, r)
        self.ug.bias.data.fill_(0)
    ...
    def forward(self, images, oimages):
        ...
        #print featuresp.size()
        featuresul = self.ul(images)
        #print featuresul.size()
        featuresug = self.ug(oimages)
        #print featuresp.size()
        #print featuresul.size()
        #print featuresug.size()
        # normalize in the joint embedding space
        if not self.no_imgnorm:
            featuresp = l2norm(featuresp, dim=-1)

        features = torch.cat((featuresp,torch.unsqueeze(oimages,1)),1)     
        
        t = torch.cat((featuresul,torch.unsqueeze(featuresug,1)),1)
        t = torch.nn.functional.softmax(t,1)

        features = torch.squeeze(torch.sum(features*t,1),1)
        if not self.no_imgnorm:
...
class SCAN(object):
    ...
    def __init__(self, opt):
        ...
        self.oimg_enc = EncoderImage(opt.data_name, opt.img_dim, opt.embed_size, finetune=opt.finetune,
                                    precomp_enc_type=opt.precomp_enc_type, cnn_type=opt.cnn_type,
                                    no_imgnorm=opt.no_imgnorm)

        #self.img_enc = nn.DataParallel(self.img_enc, device_ids=[0,1,2,3]).cuda()
        ...
        params += list(self.oimg_enc.fc.parameters())
    ...
    def state_dict(self):
        state_dict = [self.oimg_enc.state_dict(), self.txt_enc.state_dict(), self.decoder.state_dict()]
        return state_dict
    ...
    def forward_emb(self, oimages, images, captions, lengths, volatile=False):
        ...
        return img_emb, cap_emb, scores, decode_lengths, captions
    ...
    def train_emb(self, oimages, images, captions, lengths, ids=None, *args):
        ...
        img_emb, cap_emb, scores, decode_lengths, captions = self.forward_emb(oimages, images, captions, lengths)
