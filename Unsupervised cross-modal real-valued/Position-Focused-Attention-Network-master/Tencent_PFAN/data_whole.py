...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab):
        ...
        self.whole_images = np.load(loc+'%s_who_ims.npy' % data_split)
    ...
    def __getitem__(self, index):
        ...
        whole_image = torch.Tensor(self.whole_images[img_id])
        ...
        return image, whole_image, box, target, index, img_id
...
def collate_fn(data):
    ...
    images, whole_images, boxes, captions, ids, img_ids = zip(*data)

    # Merge images (convert tuple of 3D tensor to 4D tensor)
    images = torch.stack(images, 0)
    whole_images = torch.stack(whole_images, 0)
    ...
    return images, whole_images, boxes, targets, lengths, ids
