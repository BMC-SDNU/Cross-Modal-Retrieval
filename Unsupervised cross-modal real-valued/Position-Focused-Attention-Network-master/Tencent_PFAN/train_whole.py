...
from evaluation_whole import i2t, t2i, AverageMeter, LogCollector, encode_data, shard_xattn_t2i, shard_xattn_i2t
...
def main():
    ...
    parser.add_argument('--need_box_whole', action='store_true', help='Use whole image in box')
    parser.add_argument('--embed_path', default='./vocab/embedding.npy', help='Path to load the word embedding.')
    parser.add_argument('--lambda_whole', default=1., type=float, help='Lambda for whole_sim')
    ...
    vocab = deserialize_vocab(os.path.join(opt.vocab_path, '%s_sent_vocab.json' % opt.data_name))
    opt.vocab_size = len(vocab)
    print "Vocab size", opt.vocab_size

    # Load data loaders
    train_loader, val_loader = data_whole.get_loaders(
...
def validate(opt, val_loader, model):
    ...
    img_embs, whole_img_embs, cap_embs, cap_lens, final_cap_embs = encode_data(
        model, val_loader, opt.log_step, logging.info)

    print("Img shape in validate:", img_embs.shape)
    print("Whole img shape in validate:", whole_img_embs.shape)

    start = time.time()
    if opt.cross_attn == 't2i':
        sims = shard_xattn_t2i(img_embs, whole_img_embs, cap_embs, cap_lens, final_cap_embs, opt, shard_size=128)
    elif opt.cross_attn == 'i2t':
        sims = shard_xattn_i2t(img_embs, whole_img_embs, cap_embs, cap_lens, final_cap_embs, opt, shard_size=128)
    else:
