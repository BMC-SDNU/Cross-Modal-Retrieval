...
def evalrank(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        if opt.cross_attn == 't2i':
            ...
            sims = shard_xattn_i2t(img_embs, cap_embs, cap_lens, final_cap_embs, opt, shard_size=128)
        else:
...
def shard_xattn_i2t(images, whole_images, captions, caplens, final_captions, opt, shard_size=128):
    """
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            if torch.cuda.is_available():
                ...
                f_s = f_s.cuda()
            l = caplens[cap_start:cap_end]
            sim = xattn_score_i2t(im, w_im, s, l, f_s, opt)
            d[im_start:im_end, cap_start:cap_end] = sim.data.cpu().numpy()
    sys.stdout.write('\n')
    return d
