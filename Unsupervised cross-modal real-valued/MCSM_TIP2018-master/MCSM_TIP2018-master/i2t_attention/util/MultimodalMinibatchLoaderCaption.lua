...
function MultimodalMinibatchLoaderCaption.create(data_dir, nclass, img_dim, doc_length,
    ...
    --self.txt_data = matio.load(path.join(self.data_dir, 'train_txt.mat')).train_txt
    self.lab_data = matio.load(path.join(self.data_dir, 'train_lab.mat')).train_lab

    local f = hdf5.open(path.join(self.data_dir, 'wiki_txt.hdf5'), 'r')
...
function MultimodalMinibatchLoaderCaption:next_batch()
    ...
    local train_lab = self.lab_data

    --local module = nn.LookupTable(self.alphabet_size+1, 128)
