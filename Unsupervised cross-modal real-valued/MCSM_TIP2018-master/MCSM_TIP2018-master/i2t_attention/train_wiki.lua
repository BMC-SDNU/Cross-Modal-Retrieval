...
cmd:option('-checkpoint_dir', './i2t_attention/trained_models', 'output directory where checkpoints get written')
...
DocumentCNN = require('i2t_attention.modules.HybridCNNLSTM')
local ImageEncoder = require('i2t_attention.modules.ImageEncoder')
local AttentionModel = require('i2t_attention.modules.attention')
local MultimodalMinibatchLoader = require('i2t_attention.util.MultimodalMinibatchLoaderCaption')
...
if string.len(opt.init_from) > 0 then
    ...
    protos.enc_doc = DocumentCNN.cnn(opt.batch_size, loader.w2v_size, opt.emb_dim, opt.dropout, opt.avg, opt.cnn_dim)
    protos.attention = AttentionModel.atten(opt.emb_dim, opt.emb_dim, 18)
...
function feval(newpars, txt, img, labels)
    ...
    local txt_atten = protos.attention:forward(fea_txt)

    -- Criterion --
    local loss, grads = JointEmbeddingLoss(txt_atten, fea_img, labels)
    local dtxt = grads[1]       -- backprop through document CNN.
    local dimg = grads[2]       -- backprop through image encoder.

    if opt.symmetric == 1 then
        local loss2, grads2 = JointEmbeddingLoss(fea_img, txt_atten, labels)
        dtxt:add(grads2[2])       -- backprop through document CNN.
        dimg:add(grads2[1])       -- backprop through image encoder.
        loss = loss + loss2
    end

    local txt_atten_grad = protos.attention:backward(fea_txt, dtxt)

    protos.enc_doc:backward(txt, txt_atten_grad)
...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 and opt.learning_rate_decay < 1 then
