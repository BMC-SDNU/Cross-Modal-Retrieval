...
def evalrank(model, data_loader, opt, split='dev', fold5=False):
    ...
    model.eval()
    with torch.no_grad():
        img_embs, cap_ht, cap_embs, cap_lens = encode_data(model, data_loader)
        ...
        if not fold5:
            ...
            sims_0, sims_1 = shard_xattn(model, img_embs, cap_ht, cap_embs, cap_lens, opt, shard_size=128)
            if sims_1 is not None:
                sims = sims_0 + sims_1
            else:
                sims = sims_0
            ...
            for i in range(5):
                ...
                sims_0, sims_1 = shard_xattn(model, img_embs_shard, cap_ht_shard, cap_embs_shard, cap_lens_shard, opt, shard_size=128)
                if sims_1 is not None:
                    sims = sims_0 + sims_1
                else:
                    sims = sims_0
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    max_n_word = 0
    for i, (images, captions, captions_pos, lengths, masks, ids, _) in enumerate(data_loader):
        max_n_word = max(max_n_word, max(lengths))

    for i, (images, captions, captions_pos, lengths, masks, ids, _) in enumerate(data_loader):

        # compute the embeddings
        img_emb, cap_ht, cap_emb, cap_len = model.forward_emb(images, captions, captions_pos, lengths, masks)
        #print(img_emb)
        if img_embs is None:
            if img_emb.dim() == 3:
        ...
        del images, captions
    return img_embs, cap_hts, cap_embs, cap_lens

def shard_xattn(model, images, caption_ht, captions, caplens, opt, shard_size=128):
    if "vsesc" not in opt.model_mode:
        if opt.cross_attn == 't2i':
            sims = shard_xattn_t2i(model, images, caption_ht, captions, caplens, opt, shard_size=128)
        elif opt.cross_attn == 'i2t':
            sims = shard_xattn_i2t(model, images, caption_ht, captions, caplens, opt, shard_size=128)
        else:
            raise NotImplementedError
        return sims, None
    else:
        sims_0 = shard_xattn_t2i(model, images, caption_ht, captions, caplens, opt, shard_size=128)
        sims_1 = shard_xattn_i2t(model, images, caption_ht, captions, caplens, opt, shard_size=128)
        return sims_0, sims_1

def shard_xattn_t2i(model, images, caption_ht, captions, caplens, opt, shard_size=128):
    """
...
def shard_xattn_i2t(model, images, caption_ht, captions, caplens, opt, shard_size=128):
    """
