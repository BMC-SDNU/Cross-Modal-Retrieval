...
interv = 0; % 0 for observational, 1 for interventional
...
Tree = DAGlearnG_Select('tree',X,ordered,scoreType,[],A);
adjMat = Tree ~= 0;
...
k = 10;
SC = DAGlearnG_Prune_SC(X,k,ordered,A);
Enum = DAGlearnG_Select('enum',X,ordered,scoreType,SC,A);
adjMat = Enum ~= 0;
fprintf('Number of errors = %d, false positives = %d, false negatives = %d\n\n',sum(adjMat(:)~=adj(:)),sum(adjMat(:)==1 & adj(:)==0),sum(adjMat(:)==0 & adj(:) == 1));

% Greedy Search
fprintf('Running greedy method\n');
Greedy = DAGlearnG_Select('greedy',X,ordered,scoreType,[],A);
adjMat = Greedy ~= 0;
fprintf('Number of errors = %d, false positives = %d, false negatives = %d\n\n',sum(adjMat(:)~=adj(:)),sum(adjMat(:)==1 & adj(:)==0),sum(adjMat(:)==0 & adj(:) == 1));

% L1-Regularization Path
fprintf('Running L1-regularization method\n');
L1 = DAGlearnG_Select('L1',X,ordered,scoreType,[],A);
adjMat = L1 ~= 0;
fprintf('Number of errors = %d, false positives = %d, false negatives = %d\n\n',sum(adjMat(:)~=adj(:)),sum(adjMat(:)==1 & adj(:)==0),sum(adjMat(:)==0 & adj(:) == 1));
