...
from mindspore.nn import Cell, Dense, Dropout, SequentialCell, Conv2d
...
class VitStem(Cell):
    ...
    def __init__(self, vit_config):
        ...
        self.patch_to_embedding = Conv2d(channels, d_model, patch_size, stride=patch_size, has_bias=True)

    def construct(self, img):
        p = self.patch_size
        bs, channels, h, w = img.shape
        x = self.patch_to_embedding(img)
        x = self.reshape(x, (bs, channels*p*p, (h//p)*(w//p)))
