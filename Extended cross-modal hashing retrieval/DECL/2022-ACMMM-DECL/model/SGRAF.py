...
class EncoderSimilarity(nn.Module):
    ...
    # Modifications by extending DECL
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        for i in range(n_caption):
            ...
            sim_all.append(self.sim_eval_w(sim_vec))
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        self.tau = opt.tau  # Modifications by extending DECL
    ...
    def forward_sim(self, img_embs, cap_embs, cap_lens, mode='sim'):
        ...
        raw_sims = self.sim_enc(img_embs, cap_embs, cap_lens)
        if mode == 'sim':
            return torch.sigmoid(raw_sims)
        else:
            sims, evidences, sims_tanh = torch.sigmoid(raw_sims), torch.exp(
                torch.tanh(raw_sims)/self.tau), torch.tanh(raw_sims)
