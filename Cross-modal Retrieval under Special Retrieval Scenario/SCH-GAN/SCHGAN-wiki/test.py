...
database_img = open(WORKDIR + 'list/train_img.txt', 'r').read().split('\r\n')
database_txt = open(WORKDIR + 'list/train_txt.txt', 'r').read().split('\r\n')
...
def test():	
	generator_param = cPickle.load(open(GEN_MODEL_BEST_FILE))
	discriminator_param = cPickle.load(open(DIS_MODEL_BEST_FILE))
	generator = GEN(IMAGE_DIM, TEXT_DIM, HIDDEN_DIM, OUTPUT_DIM, CLASS_DIM, WEIGHT_DECAY, G_LEARNING_RATE, param=generator_param)
	...
	sio.savemat(WORKDIR + '../result/DIS_wiki_' + str(OUTPUT_DIM) + '.mat', {'B_I_tr': I_db, 'B_T_tr': T_db, 'B_I_te': I_te, 'B_T_te': T_te})
	
	I_db = extract_feature(sess, generator, database_img)
	T_db = extract_feature(sess, generator, database_txt)
	I_te = extract_feature(sess, generator, test_img)
	T_te = extract_feature(sess, generator, test_txt)
	sio.savemat(WORKDIR + '../result/GEN_wiki_' + str(OUTPUT_DIM) + '.mat', {'B_I_tr': I_db, 'B_T_tr': T_db, 'B_I_te': I_te, 'B_T_te': T_te})
