...
class GEN:
	def __init__(self, image_dim, text_dim, hidden_dim, output_dim, class_dim, weight_decay, learning_rate, param=None):
		...
		self.image_label = tf.placeholder(tf.int32, shape=[None], name="image_label")
		self.text_data = tf.placeholder(tf.float32, shape=[None, self.text_dim], name="text_data")
		self.text_label = tf.placeholder(tf.int32, shape=[None], name="text_label")
		
		self.image_onehot_label = tf.one_hot(indices = self.image_label, depth = self.class_dim)
		self.text_onehot_label = tf.one_hot(indices = self.text_label, depth = self.class_dim)
		...
		self.gen_loss = tf.reduce_mean(tf.abs(self.gen_prob - self.reward)) \
						+ tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = self.image_class_pred, labels = self.image_onehot_label)) \
						+ tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = self.text_class_pred, labels = self.text_onehot_label)) \
